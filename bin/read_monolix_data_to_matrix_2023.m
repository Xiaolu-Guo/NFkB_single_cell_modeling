function data = read_monolix_data_to_matrix_2023(data_proj_num_vec,data_parent_path,cell_num_data)
% input
...
data.para_name=cell(2,1);
data.parameters_mode=cell(2,1);
data.parameters_mode_nan=cell(2,1);
...
for i_proj = 1:length(data_proj_num_vec)
    ...
    [proj_path,proj_name] = subfunc_get_proj_path_2(proj_num,data_parent_path,'XGESN2023');
    ...
    [parameters,para_name,~] = read_draw_individual_parameters_mode_2023(proj_num,data_parent_path);
    ...
    for i_sti=1:length(est_info_proj.data.ligand_vec)
        ...
        data.parameters_mode{i_data} = NaN(cell_num, size(parameters,2));
        ...
        for i_cell_each_sti=id_start(i_sti):id_end(i_sti)
            ...
            data.parameters_mode{i_data}(i_cell,:) = parameters(i_cell_each_sti,:);
            i_cell=i_cell+1;
        end
        ...
        data.parameters_mode_nan{i_data} = data.parameters_mode{i_data}(~sum(isnan(data.pred_mode{i_data}),2),:);
        
        data.para_name{i_data} = para_name;
