function [sim_data_tbl] = NFkB_signaling_para_conc_sim(sim_info,data_info)
...
for i_ligand = 1:length(sim_info.ligand)
    dose_scale(i_ligand) = get_stimuli_info_dose_scale(sim_info.ligand(i_ligand)); % Convert to uM (from nM)
...
for i_para_val = 1:size(sim_info.parameter_value_vec,2)% 10%
    ...
    if isempty(output)
        options=options0;
        [~,x,simdata] = nfkbSimulate_2023(stim_input,names, [], {},options);
    else
        options.STEADY_STATE = simdata.STEADY_STATE;
        [~,x] = nfkbSimulate_2023(stim_input,names, [], {},options);
    end
