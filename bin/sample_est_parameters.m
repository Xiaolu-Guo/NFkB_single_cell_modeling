function [para_val,estimates_params_sample] = sample_est_parameters(proj_num,Num_sample,data_parent_path,var_fold)
...
for i_para = 1:length(estimates_params_sample.name)
    ...
    for j_para = 1:length(estimates_params_sample.name)
        if i_para~=j_para
            ...
            index_corr = strcmp(estimates_cell.Fixed,corr_name1)|strcmp(estimates_cell.Fixed,corr_name2);
            if sum(index_corr)~=1
                error('there is no corresponding correlationship')
            else
                estimates_params_sample.corr(i_para,j_para) = estimates_cell.x____________________________(index_corr);
            end
        end
    end
end
