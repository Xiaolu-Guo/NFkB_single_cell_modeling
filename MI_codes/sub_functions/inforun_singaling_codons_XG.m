function [info, sc_info] = inforun_singaling_codons_XG(all_dims, sc_dims, sc_names, high_dim, info)
%%
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
depth = 1; % Candidates to carry forward at each dimension increase
...
% Calculate CC for signaling codons
disp('running signaling codon cc calculation')
sc_info = [];
X= cell(size(sc_dims));
for expt =1:length(sc_dims)
    X{expt} = sc_dims{expt}';
end
tic
[fitI,I,~,Q] = jacknifeCC(X,30,12);
...
sc_info(1).I = all_I;
sc_info(1).Q = all_Q;
sc_info(1).names = sc_names;
sc_info(1).idx{1} = (1:size(sc_dims{1},2))';
t = toc;
disp(['signaling codons I = ',num2str(fitI), ' (t = ',num2str(round(t*100)/100),' sec)'])
save sc_info.mat sc_info
...
%                     X{expt} = cat(1, sc_dims{expt}', all_dims{expt}(:,dim1)');
%                 end
...
%                     X= cell(size(all_dims));
%                     for expt =1:length(all_dims)
%                         X{expt} = cat(1, sc_dims{expt}', all_dims{expt}(:,dim_idx(j,:))');
%                     end
%                     [fitI,I,~,Q] = jacknifeCC(X,9,12)
