...
% some predicates
is_normal_matrix = @(x) isnumeric(x) && ismatrix(x) && isreal(x) && ~issparse(x);
...
%require_arg(k <= n, ['The value k exceeds the number of reference
%points');
require_arg(k <= n, ['The value k exceeds the number of reference points', 'number of reference points ', num2str(n)]);   %added by AS 11/23/2021 for debugging       
