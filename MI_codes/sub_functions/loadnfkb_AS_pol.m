function [nfkb, all_dims, sc_dims, names_1D, names_sc_1D] = loadnfkb_AS_pol(stimuli)
%%
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
for i = 1:length(polarization)
    ids{count} = strcat(stimuli,polarization{i});
    count = count + 1;
end
...
P2 = mfilename;
nfkb_name = [P(1:(length(P)-length(P2))) , stimuli, '_nfkb.mat'];
