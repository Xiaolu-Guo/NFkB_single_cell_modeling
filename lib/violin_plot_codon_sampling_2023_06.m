function [] = violin_plot_codon_sampling_2023_06(collect_feature_vects,figure_option)
...
for i=1:length(codewords)
    ...
    id_begin=1;
        ...
        position_subfig(1)=(0.15+width_shift)*subplot_width;%*(jj~=1)
        ...
        if plot_ornot
            ...
            vects_index = id_begin:id_end;
            vects_len = length(vects_index);
            
% important!!!!!!! need to be check which one is sampling vs experiment
%             vects_index(1:2:vects_len) = vects_index_0(2:2:vects_len);
%             vects_index(2:2:vects_len) = vects_index_0(1:2:vects_len);

            plot_pos = id_begin:id_end;
            plot_pos(1:2:vects_len) = plot_pos(1:2:vects_len)+0.2;
            plot_pos(2:2:vects_len) = plot_pos(2:2:vects_len)-0.2;

            
            violin_20230616_for_sampling(data_type,vects(vects_index), plot_pos, 'Axes', ax, 'YLim', ylimits,'Color',{[0,1,0],[0,0,0]});
            
            if heigh_pos == 0
                %title(codewords{i})
                set(ax,'XTick',(id_begin+.5):2:id_end,'XTickLabel',ligand_all);
