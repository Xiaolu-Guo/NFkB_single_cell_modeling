function [] = violin_plot_codon_compare_co_sti_202306(collect_feature_vects,figure_option)
...
for i=1:length(codewords)
    ...
    for jj=1:length(ligand_all)
        ...
        if plot_ornot
            ...
            position_subfig(2)=(heigh_pos*2+(length(data_type)-i_data_type)+0.1+height_shift)*subplot_height;
            ...
            %ax=gca;
            id_begin = find(strcmp(collect_feature_vects.info_ligand,ligand_all{jj})&...
                strcmp(collect_feature_vects.info_data_type,data_type_all{i_data_type}),1,'first');
            id_end = find(strcmp(collect_feature_vects.info_ligand,ligand_all{jj})&...
                strcmp(collect_feature_vects.info_data_type,data_type_all{i_data_type}),1,'last');
            ...
            violin_20230616(data_type_each_plot,vects(id_begin:id_end), plot_pos, 'Axes', ax, 'YLim', ylimits,'Color',colors(i_data_type),'Area',0.03);
            
            set(ax,'XTick',(id_begin+.5):2:id_end,'XTickLabel',{});
            
            if heigh_pos == 0 && (length(data_type)-i_data_type) == 0
                %title(codewords{i})
