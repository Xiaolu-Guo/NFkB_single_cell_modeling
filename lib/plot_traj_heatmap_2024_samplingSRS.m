function [] = plot_traj_heatmap_2024_samplingSRS(data,vis_data_field,savepath,filter_TNF,order_name)
...
for i_sti = 1:length(data.(vis_data_field{1}))
    for i_data_field = 1:length(vis_data_field)
        ...
        saveas(gcf,strcat(savepath,data.info_ligand{i_sti},'_',replace(data.info_dose_str{i_sti},'/',''),'_',vis_data_field{i_data_field},fitting_dose,'_',order_name),'epsc');
        close
    end
end
