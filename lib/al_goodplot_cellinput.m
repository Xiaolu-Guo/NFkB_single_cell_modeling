function [h, mu, sigma, q, notch] = al_goodplot_cellinput(x, pos, boxw, col, type, bw, p, jitw)
% Violin and box plots for visualization of data distribution.
%
%   Inputs:
%     - x: 1xP cell, each cell Nx1 (N can be diff) vector, data (P plots) (sample data if empty).
...
if nargin<1 || isempty(x)
    error('not support empty input')
end
if nargin<2 || isempty(pos)
    pos=1:length(x);
end

allData = vertcat(x{:}); % Combine all data to compute global standard deviation
...
u = 0.9*min(allData):p:1.1*max(allData);
...
for i_x = 1:length(x)
    if size(x{i_x},1)==1, x=x'; end
end
if length(x)>1 && size(pos,1)==1, pos=repmat(pos,1,size(x,2)); end
if length(x)>1 && size(col,1)==1, col=repmat(col,size(x,2),1); end
if length(x)>1 && size(boxw,1)==1, boxw=repmat(boxw,1,size(x,2)); end
for i=1:length(x)
    ...
    switch type
        ...
        case 'left'
            ...
            plot([pos(i)-boxw(i)/4 pos(i)], [mu(i) mu(i)],'color',col(i,:)/2,'linewidth',1.5,'HandleVisibility','off')
