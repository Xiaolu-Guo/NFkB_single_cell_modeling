% checking the CpGen and polyICen
% clear all
function [] = para_sample_fitting_weighted_match_CpG_PolyIC_Compete_202401(data_save_file_path,input_paras,cal_codon,save_metric_name,fitting_dose)
...
for i_proj_num_vec = 1:length(proj_num_vec)
    ...
    switch length(proj_num_vec{i_proj_num_vec})
        ...
        otherwise
            % sample bi-stimulation
                ...
                [para_sample_multi_ligand,estimates_2ligand] = sample_fitting_weighted_match_multi_receptor_2023_12(proj_num_vec{i_proj_num_vec},Num_sample(i_proj_num_vec),data_fitting,data_fitting_all,sim_info,'alldose');
    ...
    data_info.species_outputname = {'nucNFkB';'CpG_en';'polyIC_en';'TLR2';'TLR3';'TLR9';'IKK';'TAK1';'IkBamRNA'};
    data_info.species_composition = {{'NFkBn';'IkBaNFkBn'};{'CpG_en'};{'polyIC_en'};{'TLR2'};{'TLR3'};{'TLR9'};{'IKK'};{'TAK1'};{'IkBat'}};
    data_info.save_file_name = save_filename; % only beginning, no .mat
    
    sim_data_tbl = genotype_sim_save_2023_CpGpolyIC_compete(sim_info,data_info,gene_info);
    ...
    if cal_codon
        ...
        for i_ligand = 2:length(proj_ligand_vec{i_proj_num_vec})
            ...
            dose_str = strcat(dose_str,'_',proj_dose_str_vec{i_proj_num_vec}{i_ligand});
        end
        ...
        data.info_num_cells{i_proj_num_vec} = size(sim_data_tbl.trajectory,1)/length(data_info.species_outputname);
...
if cal_codon
    ...
    % end
    save(strcat(data_save_file_path,save_metric_name),'data','metrics','collect_feature_vects','sim_data_tbl');
