...
% clear all
function [] = para_fitting_sim_IkBao_202401(para6fold,data_save_file_path,input_paras,cal_codon,save_metric_name)
...
for i_proj_num_vec = 1:length(proj_num_vec)
    ...
    if length(proj_num_vec{i_proj_num_vec})==1
        ...
        gene_info.parameter_name_vec = {{est.name{parameter_index},'params6'}};
        gene_info.gene_parameter_value_vec_genotype{1}{1} = [para_val(:,parameter_index)';
            ones(1,size(para_val,1))*6e-05*para6fold];
