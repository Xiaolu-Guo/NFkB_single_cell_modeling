function [t,x, simdata, x_end] = nfkbSimulate_CpGPolyIC_compete(stimulus, names, params_mod, init_mod, options)
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
if nargin <4
    ...
    if nargin < 3
        params_mod = [];
        if nargin <2
            [~, tmp] = nfkbInitialize_CpGPolyIC_compete;
            names = tmp.NAMES;
            if nargin<1
                stimulus = {};
            end
...
if ~isfield(v,'STEADY_STATE')
    v.PHASE     = 1;
    [~, r1] = ode15s('nfkbOde_CpGPolyIC_compete', [v.T_EQUILIBRATE 0], v.SPECIES.INIT,ode_opt,v);
    ...
    while ~converge
        [~, r1] = ode15s('nfkbOde_CpGPolyIC_compete', [v.T_EQUILIBRATE 0], starting_vals,ode_opt,v);
...
v.PHASE = 2;
[t_sim, x_sim] = ode15s('nfkbOde_CpGPolyIC_compete', v.SIM_TIME,starting_vals,ode_opt,v);
