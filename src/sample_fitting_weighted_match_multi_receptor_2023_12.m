    ...
    for i_para = 1:size(para_sample_ligand{i_ligand},1)
        ...
        for j_ligand = 1:length(proj_num)
            if i_ligand ~= j_ligand
                origin_diff = (para_sample_ligand{i_ligand}(i_para,index_core_ligand{i_ligand}) - para_sample_ligand_large{j_ligand}(:,index_core_ligand{j_ligand}));
                weighted_diff = zeros(size(origin_diff));
                weighted_diff(:,1) = origin_diff(:,1)/(10-0.1);
                weighted_diff(:,2) = origin_diff(:,2)/(1-0.01);
                weighted_diff(:,3) = origin_diff(:,3)/(1-0.01);
                weighted_diff(:,4) = origin_diff(:,4)/(0.3-0.04);
                diff = sum((weighted_diff).^2,2);
