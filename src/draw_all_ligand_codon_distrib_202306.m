function [] = draw_all_ligand_codon_distrib_202306(data_save_file_path,fig_save_path)
...
for i_ligand = 1:length(ligand_vec)%TNF,LPS [27];%
    ...
    for i_dose = 1:length(dose_vec{i_ligand})
        ...
        good_fit_pop_l1(i_ligand,i_dose) = mean(l1_diff_kde(:,i_sti));
        good_fit_pop_JSD(i_ligand,i_dose) = mean(JSD_diff(:,i_sti));
...
if change_order
    i_ligand_order = 1:lenght(sti_vec);
...
if 0 % l2 norm
    ...
    saveas(gcf,strcat(fig_save_path,'codon_diff_202306'),'epsc')
    close()
end
...
if 0 % JSD
    ...
    saveas(gcf,strcat(fig_save_path,'codon_diff_jsd_202306'),'epsc')
    close()
end
...
if 1
    ...
    % violin_plot_codon_2023_06(collect_feature_vects,fig_opt) % draw all codon and sti in one
