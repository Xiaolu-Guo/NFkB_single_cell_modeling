function [] = draw_spearman_corr_each_codon_2023_03(data_save_file_path,fig_save_path)
    ...
    for i_feature = 1:length(features)
        ...
        for i_sti = 1:length(ligand_vec)
            ...
            for i_dose = 1:length(dose_vec{i_sti})
                ...
                sti_vec{i_cond} = strcat(ligand,'-',dose);
        ...
        if 0
            ...
            h = heatmap( sti_vec,para_names,corr_rel,'Colormap',mymap,'CellLabelColor','none');%'k' % corr_mat_codon
            ...
            saveas(gcf,strcat(fig_save_path,features{i_feature},'_para_corr'),'epsc')
            close()
        end
        
        if 1
            pval_corr = (pval_mat_codon <=0.05)*1;
            ...
            paper_size = [450,200];
            set(gcf, 'PaperPosition', paper_pos,'PaperSize',paper_size )%,'Position',draw_pos [20,20,280,280]
            h = heatmap( sti_vec,para_names,pval_corr,'Colormap',mymap,'CellLabelColor','none');%'k' % corr_mat_codon
            xlabel(features{i_feature})
            caxis([0,1])
            %h.CellLabelColor = [0,0,0];
            h.CellLabelFormat = '%0.2g';
            
            saveas(gcf,strcat(fig_save_path,'pval_corr_',features{i_feature},'_para_corr'),'epsc')
            close()
        end
...
if 1
    ...
    for i_feature = 1:length(features)
        ...
        if 0
            ...
            h.CellLabelFormat = '%0.2g';
            colorbar('off')
            saveas(gcf,strcat(fig_save_path,'pval_corr_',features{i_feature},'_para_corr_high_dose'),'epsc')
            close()
        end
    ...
    for i_para = 1:size(corr_mat_para,3)
        ...
        paper_size = [200,150];
        set(gcf, 'PaperPosition', paper_pos,'PaperSize',paper_size )%,'Position',draw_pos [20,20,280,280]
        h = heatmap( sti_vec2,features,corr_mat_para(:,:,i_para),'Colormap',mymap,'CellLabelColor','none');%'k' % corr_mat_codon
        % xlabel(para_names{i_para})
        caxis([-1,1])
        %h.CellLabelColor = [0,0,0];
        h.CellLabelFormat = '%0.2g';
             colorbar('off')
        ...
        saveas(gcf,strcat(fig_save_path,para_names{i_para},'_codon_corr_high_dose'),'epsc')
        close()
    end
    end
end
