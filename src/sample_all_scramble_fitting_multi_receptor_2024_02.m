function [para_sample_multi_ligand,estimates_2ligand]= sample_all_scramble_fitting_multi_receptor_2024_02(proj_num,Num_sample,data,data_all,sim_info,fitting_dose)
    ...
    for i_para = 1:size(para_sample_ligand{i_ligand},1)
        ...
        for j_ligand = 1:length(proj_num)
            if i_ligand ~= j_ligand
                ...
                scramble_index_1 = randperm(size(para_sample_ligand_large{j_ligand},1),1);
%                 para_val(j_para,index_rcp{j_ligand}) = ...
%                     para_sample_ligand_large{j_ligand}(min_index_1,index_rcp_ligand{j_ligand});
                para_val(j_para,index_rcp{j_ligand}) = ...
                    para_sample_ligand_large{j_ligand}(scramble_index_1,index_rcp_ligand{j_ligand});
            end
        end
        j_para = j_para+1;
    end
...
para_sample_multi_ligand = zeros(size(para_val));

para_val_size = size(para_val,1)
for i = 1:size(para_val,2)
    % Generate a permutation of para_val for each column
    para_sample_multi_ligand(:, i) = para_val(randperm(para_val_size),i);
end
a = 1;