function [] = draw_all_ligand_codon_distrib_202401(data_save_file_path,fig_save_path)
...
if 1
    ...
    w_dis_codon = NaN(90,90);
    for i_sti = 1:15
        for j_sti = 1:15
            w_dis_codon(((i_sti-1)*6+1):(i_sti*6),((j_sti-1)*6+1):(j_sti*6)) = diag(reshape(W_diff(i_sti,j_sti,:),[1,6]));
        end
    end
    ...
    h1 = heatmap(w_dis_codon,'colormap',mymap,'ColorbarVisible','off')
    caxis([0,0.2])
    
    %         minDataValue = min(w_dis_codon(:), [], 'omitnan');
    % maxDataValue = max(w_dis_codon(:), [], 'omitnan');
    % h1.ColorLimits = [minDataValue, maxDataValue];
    ...
    saveas(gcf,strcat(fig_save_path,'Wdis_exp_sim_allcodon_202401'),'epsc')
    close()
