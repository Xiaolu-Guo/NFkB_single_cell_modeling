function [sim_data_tbl] = NFkB_signaling_para_conc_sim_2023_CpGpolyIC_compete(sim_info,data_info)
...
[v0.PARAMS, v0.SPECIES] = nfkbInitialize_CpGPolyIC_compete();
...
for i_para_val = 1:size(sim_info.parameter_value_vec,2)% 10%
    ...
    if isempty(output)
        options=options0;
        [~,x,simdata] = nfkbSimulate_CpGPolyIC_compete(stim_input,names, [], {},options);
    else
        options.STEADY_STATE = simdata.STEADY_STATE;
        [~,x] = nfkbSimulate_CpGPolyIC_compete(stim_input,names, [], {},options);
    end
