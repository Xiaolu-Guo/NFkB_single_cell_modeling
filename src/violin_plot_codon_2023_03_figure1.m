function [] = violin_plot_codon_2023_03_figure1(collect_feature_vects,figure_option)
...
codewords = {'PeakAmplitude','TotalActivity','OscVsNonOsc'}
ligand_all = unique(collect_feature_vects.info_ligand,'stable');
ligand_all = {'TNF';'LPS';'CpG'};
units_all = {'ng/mL';'ng/mL';'nM'};
dose_val = {{'10'};
    {'100'};
    {'1000'}};
...
height_shift=0.7;
width_shift=0.25;
subplot_width=1/(5+width_shift);
subplot_height=1/(6+height_shift);%length(codewords)
...
for i=1:length(codewords)
    ...
    for jj=1% :length(ligand_all)
        ...
        position_subfig(1)=(1-1+0.15+width_shift)*subplot_width;%*(jj~=1)
...
%                 & strcmp(collect_feature_vects.info_ligand,ligand_all{jj}));% LPS
            ...
            id_vec = [5,6,15,16,25,26];
            violin(data_type,vects(id_vec), id_begin:id_end, 'Axes', ax, 'YLim', ylimits);
