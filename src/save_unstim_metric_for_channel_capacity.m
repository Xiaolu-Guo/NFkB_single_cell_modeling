...
%load the unstim experiment, already rescaled 
...
clear data
i_sti = 1;
[row,col] = find(isnan(data_exp));
non_nan_index = setdiff(1:size(data_exp,1),row);
data.exp{i_sti} = data_exp(non_nan_index,1:97);
data.info_ligand{i_sti} = 'unstim';
data.info_dose_str{i_sti} = '0';
% cd(codon_info.codon_path)
vis_data_field = {'exp'};%,'sample'};
data_label = {'experiments'};%,'sample'};

% calculate the metrics for the unstim
...
%% load simulation data for fitting and sampling
...
load(strcat(data_save_file_path,'Sim_unstim_fitting_alldose_r2_codon_metric.mat'))

metric_names = fieldnames(metrics{1});

for i_metric_name = 1:length(metric_names)
    metics_sim_unstim_sampling{1}.(metric_names{i_metric_name}) = [];
    for i_metric = 1:length(metrics)
        
        metics_sim_unstim_sampling{1}.(metric_names{i_metric_name}) = [metics_sim_unstim_sampling{1}.(metric_names{i_metric_name});metrics{i_metric}.(metric_names{i_metric_name})(1:9:end,:)];
    end
end
...
%% combine all the experimental data, fitting data, sampling data, and calculate the signaling codon together
i_ids = 1;
...
for i_metric_name = 1:length(metric_names)
    ...
    data_info.info_ligand{i_metric_index} = 'unstim';
    ...
    metrics_all{i_metric_index}.(metric_names{i_metric_name}) = metics_sim_unstim_sampling{1}.(metric_names{i_metric_name})(:,:);
    data_info.info_ligand{i_metric_index} = 'unstim';
    data_info.info_dose_str{i_metric_index} = '0';
    data_info.data_label{i_metric_index} = 'sampling';
...
%% save the trajectory data to mutual info format
...
for i_data_set = 1:length(data_name)
    ...
    ligand_all = unique(data_info.info_ligand(index_data),'stable') ;

    for i_ligand = 1:length(ligand_all)
        nfkb(i_ligand).sc_metrics = struct();
        nfkb(i_ligand).metrics = struct();
        nfkb(i_ligand).id = ligand_all{i_ligand};
        nfkb(i_ligand).ids = ligand_all;
        nfkb(i_ligand).metrics.time_series = metrics_all{index_data(i_ligand)}.time_series;
    end
    
    save(strcat('mutual_info_format_traj_single_ligand_',data_name{i_data_set},'_20230614.mat'),'nfkb')
...
for i_data_set = 1:length(data_name)
    ...
    save(strcat('mutual_info_format_codon_single_ligand_unstim_',data_name{i_data_set},'_20230614.mat'),'nfkb')
