function [] =  draw_traj_RMSD(data_save_file_path,fig_save_path)

for data_proj_num_vec=[18 ,25:28]%TNF,LPS [27];%
    data = read_monolix_data_to_matrix(data_proj_num_vec,data_save_file_path);
    
    
    paperpos = [0,0,150,50]*1.8;
    papersize = [150,50]*1.8;
    draw_pos = [10,10,140,30]*1.8;
    ...
    for i_sti = 1:length(data.pred_mode)
        ...
        histogram(RMSD_TNF,'Normalization','pdf'); hold on
        ...
        for i_plot =1:3
            ...
            xlabel('time')
            ylabel('NFkB')
            title(strcat(num2str(pct(i_plot)*100),'%'))
