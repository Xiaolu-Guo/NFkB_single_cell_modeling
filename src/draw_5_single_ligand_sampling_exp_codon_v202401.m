...
for i_vers = 1:4
    ...
    if 1
        ...
        for i_cond = 1:length(index_exp_ligand_vects)
            ...
            for i_rpc = 1:length(index_exp_ligand)
                ...
                data_info.info_ligand(i_index:i_index+3) =[data_exp.info_ligand(index_exp_ligand(i_rpc)),...
                    data_sim_weighted_match.info_ligand(i_cond),...
                    data_scrambled_sim.info_ligand(i_cond),...
                    data_sim.info_ligand(i_cond)];
                
                data_info.info_dose_str(i_index:i_index+3) =[data_exp.info_dose_str(index_exp_ligand(i_rpc)),...
                    data_sim_weighted_match.info_dose_str(i_cond),...
                    data_scrambled_sim.info_dose_str(i_cond),...
                    data_sim.info_dose_str(i_cond)];
                
                data_info.data_label(i_index:i_index+3) = {'exp','sim_weighted_match','scramble_sim','sim'};
    ...
    W_diff_comp = [W_diff_scrambled_sim_minmax_mat;W_diff_sim_minmax_mat;W_diff_weighted_match_sim_minmax_mat]';
    W_diff_comp_codon = [W_diff_scrambled_sim_minmax_mat_codon,W_diff_sim_minmax_mat_codon,W_diff_weighted_match_sim_minmax_mat_codon];
...
Xlabel_codon = reordercats(Xlabel_codon,{'Speed','PeakAmplitude','Duration' ,'TotalActivity', 'EarlyVsLate','OscVsNonOsc'});
    ...
    if 1 % W-distance, run this for figure
        ...
        lgd = legend({'Scramble','Matched','Weighted Matched'},'Location','NorthEast');
        set(lgd,'Box','off')
        ylabel('W-distance')
        ylim([0,0.2])
        ...
        lgd = legend({'Scramble','Matched','Weighted Matched'},'Location','NorthWest');
        set(lgd,'Box','off')
        ylabel('W-distance')
        ylim([0,0.2])
